...
另外：本补丁会把上游四种协作模式（Plan / Code / Pair / Execute）的默认 `reasoning_effort` **统一调整为 `xhigh`**。

⚠️ macOS 15（Sequoia）上如果用 `cp` 直接覆盖 vendor 里的 `codex`，可能会出现 `killed`（exit=137）。本脚本用“临时文件 → `mv` 原子替换”的方式安装，并在安装后执行一次 `--version`；失败会自动回滚。
